...
#[repr(C)]
pub struct AddCarryOperation<T> {
    /// The result of `a + b`.
    pub value: Word<T>,
...
impl<F: Field> AddCarryOperation<F> {
    pub fn populate(
        &mut self,
        ...
        let expected = a_u32.wrapping_add(b_u32).wrapping_add(c);
        ...
        if (a[0] as u32) + (b[0] as u32) + c > 255 {
            carry[0] = 1;
            self.carry[0] = F::ONE;
        }
    ...
    pub fn eval<AB: ZKMAirBuilder>(
        ...
        let overflow_0 = a[0] + b[0] - cols.value[0] + c;
