...
impl Instruction {
    ...
    pub fn decode_from(insn: u32) -> anyhow::Result<Self> {
        ...
        match (opcode, func) {
            ...
            // } // SLL: rd = rt << sa
            (0b000000, 0b000000) => Ok(Self::new(Opcode::SLL, rd, rt, sa, false, false)), // SLL: rd = rt << sa
            // (0b000000, 0b000010) => {
            //     if rs == 1 {
            ...
            // } // DIV: (hi, lo) = rt / rs
            (0b000000, 0b011010) => Ok(Self::new(Opcode::DIV, rd, rt, rs, false, false)), // DIV: (hi, lo) = rt / rs
            ...
            //     rd,
            // )), // DIVU: (hi, lo) = rt / rs
            (0b000000, 0b011011) => Ok(Self::new(Opcode::DIVU, rd, rt, rs, false, false)), // DIVU: (hi, lo) = rt / rs
            // (0b000000, 0b010000) => {
            ...
            (0x01, _) => {
                if rt == 1 {
                    ...
                    Ok(Self::new(Opcode::BGE, rs as u8, 032, offset, false, true))
                } else if rt == 0 {
                    // Ok(Operation::Branch(BranchCond::LT, rs, 0u8, offset)) // BLTZ
                    Ok(Self::new(Opcode::BLT, rs as u8, 0u32, offset, false, true))
            ...
            (0x06, _) => Ok(Self::new(Opcode::BLE, rs as u8, 0u32, offset, false, true)), // BLEZ
            // (0x07, _) => Ok(Operation::Branch(BranchCond::GT, rs, 0u8, offset)),         // BGTZ
            (0x07, _) => Ok(Self::new(Opcode::BGT, rs as u8, 0u32, offset, true, true)), // BGTZ

            // (0b100000, _) => Ok(Operation::MloadGeneral(MemOp::LB, rs, rt, offset)),
            (0b100000, _) => Ok(Self::new(Opcode::LB, rs as u8, rt, offset, false, true)),
            // (0b100001, _) => Ok(Operation::MloadGeneral(MemOp::LH, rs, rt, offset)),
            (0b100001, _) => Ok(Self::new(Opcode::LH, rs as u8, rt, offset, false, true)),
            // (0b100010, _) => Ok(Operation::MloadGeneral(MemOp::LWL, rs, rt, offset)),
            (0b100010, _) => Ok(Self::new(Opcode::LWL, rs as u8, rt, offset, false, true)),
            // (0b100011, _) => Ok(Operation::MloadGeneral(MemOp::LW, rs, rt, offset)),
            (0b100011, _) => Ok(Self::new(Opcode::LW, rs as u8, rt, offset, false, true)),
            // (0b100100, _) => Ok(Operation::MloadGeneral(MemOp::LBU, rs, rt, offset)),
            (0b100100, _) => Ok(Self::new(Opcode::LBU, rs as u8, rt, offset, false, true)),
            // (0b100101, _) => Ok(Operation::MloadGeneral(MemOp::LHU, rs, rt, offset)),
            (0b100101, _) => Ok(Self::new(Opcode::LHU, rs as u8, rt, offset, false, true)),
            // (0b100110, _) => Ok(Operation::MloadGeneral(MemOp::LWR, rs, rt, offset)),
            (0b100110, _) => Ok(Self::new(Opcode::LWR, rs as u8, rt, offset, false, true)),
            // (0b110000, _) => Ok(Operation::MloadGeneral(MemOp::LL, rs, rt, offset)),
            (0b110000, _) => Ok(Self::new(Opcode::LL, rs as u8, rt, offset, false, true)),
            // (0b101000, _) => Ok(Operation::MstoreGeneral(MemOp::SB, rs, rt, offset)),
            (0b101000, _) => Ok(Self::new(Opcode::SB, rs as u8, rt, offset, false, true)),
            // (0b101001, _) => Ok(Operation::MstoreGeneral(MemOp::SH, rs, rt, offset)),
            (0b101001, _) => Ok(Self::new(Opcode::SH, rs as u8, rt, offset, false, true)),
            // (0b101010, _) => Ok(Operation::MstoreGeneral(MemOp::SWL, rs, rt, offset)),
            (0b101010, _) => Ok(Self::new(Opcode::SWL, rs as u8, rt, offset, false, true)),
            // (0b101011, _) => Ok(Operation::MstoreGeneral(MemOp::SW, rs, rt, offset)),
            (0b101011, _) => Ok(Self::new(Opcode::SW, rs as u8, rt, offset, false, true)),
            // (0b101110, _) => Ok(Operation::MstoreGeneral(MemOp::SWR, rs, rt, offset)),
            (0b101110, _) => Ok(Self::new(Opcode::SWR, rs as u8, rt, offset, false, true)),
            // (0b111000, _) => Ok(Operation::MstoreGeneral(MemOp::SC, rs, rt, offset)),
            (0b111000, _) => Ok(Self::new(Opcode::SC, rs as u8, rt, offset, false, true)),
            // (0b111101, _) => Ok(Operation::MstoreGeneral(MemOp::SDC1, rs, rt, offset)),
            (0b111101, _) => Ok(Self::new(Opcode::SDC1, rs as u8, rt, offset, false, true)),
            ...
            //     offset,
            // )), // ADDI: rt = rs + sext(imm)
            (0b001000, _) => Ok(Self::new(Opcode::ADDI, rt as u8, rs, offset, false, true)), // ADDI: rt = rs + sext(imm)
            ...
            (0b001001, _) => Ok(Self::new(Opcode::ADDIU, rt as u8, rs, offset, false, true)), // ADDIU: rt = rs + sext(imm)
            ...
            //     offset,
            // )), // SLTI: rt = rs < sext(imm)
            (0b001010, _) => Ok(Self::new(Opcode::SLTI, rt as u8, rs, offset, false, true)), // SLTI: rt = rs < sext(imm)
            ...
            (0b001011, _) => Ok(Self::new(Opcode::SLTIU, rt as u8, rs, offset, false, false)), // SLTIU: rt = rs < sext(imm)
